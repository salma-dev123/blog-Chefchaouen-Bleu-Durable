    ...
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                <h2 class="text-2xl font-semibold mb-6">Modifier l'article</h2>
                ...
                <form action="{{ route('articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="slug" id="slug" value="{{ old('slug', $article->slug) }}" data-initial-slug="{{ old('slug', $article->slug) }}"
                               class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                               required>
                        @error('slug')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <textarea name="extrait" id="extrait" rows="3" 
                                  class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"
                                  required>{{ old('extrait', $article->extrait) }}</textarea>
                        @error('extrait')
                            <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="bg-white border border-gray-200 rounded-xl overflow-hidden">
                          <div id="hs-editor-tiptap">
                            ...
                            <div class="h-40 overflow-auto p-4 text-gray-800" data-hs-editor-field contenteditable="true">{!! old('contenu', $article->contenu) !!}</div>
                            
                            <!-- Champ caché pour le formulaire -->
                            <textarea name="contenu" id="contenu" class="hidden">{{ old('contenu', $article->contenu) }}</textarea>
                            ...
                            <div class="space-y-2">
                                <div class="flex items-center">
                                    <input id="status-published" name="status" type="radio" value="publie" 
                                           ...
                                           {{ old('status', $article->status) == 'publie' ? 'checked' : '' }} required>
                                ...
                                <div class="flex items-center">
                                    <input id="status-draft" name="status" type="radio" value="brouillon" 
                                           class="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300"
                                           {{ old('status', $article->status) == 'brouillon' ? 'checked' : '' }}>
